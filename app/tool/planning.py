...
class PlanningTool(BaseTool):
    ...
    parameters: dict = {
        ...
        "required": [],  # Command is optional - will be inferred if missing
    ...
    def _log_to_monitor(self, message: str):
        """Log to E2B activity monitor if we're in E2B sandbox"""
        try:
            import subprocess

            subprocess.run(
                f"echo '{message}' >> /tmp/agent_activity.log",
                shell=True,
                capture_output=True,
                timeout=1,
            )
            # For TestOpsAI: Keep browser focused (no Activity Monitor)
            # Just log to activity file, don't change window focus
            # Browser stays as primary window throughout execution
        except:
            pass  # Silently fail if not in E2B or log unavailable
    ...
    async def execute(
        ...
        **kwargs,
        ...
        # Infer command if missing (for Gemini API compatibility)
        if not command:
            if step_index is not None:
                command = "mark_step"
            elif title and steps:
                command = "create" if plan_id not in self.plans else "update"
            elif not plan_id:
                command = "list"
            else:
                command = "get"
    ...
    def _create_plan(
        ...
        if plan_id in self.plans:
            # If plan already exists, return the existing plan instead of erroring
            return ToolResult(
                output=f"Plan '{plan_id}' already exists. Returning existing plan.\n\n{self._format_plan(self.plans[plan_id])}"
            )
        ...
        # Log to E2B activity monitor if available
        self._log_to_monitor(f"[PLAN] Created: {title} ({len(steps)} steps)")
    ...
    def _mark_step(
        ...
        if step_status:
            ...
            # Log status changes to activity monitor
            step_name = plan["steps"][step_index][:50]
            status_prefix = {
                "not_started": "[PENDING]",
                "in_progress": "[ACTIVE]",
                "completed": "[DONE]",
                "blocked": "[BLOCKED]",
            }.get(step_status, "[STEP]")
            self._log_to_monitor(
                f"{status_prefix} Step {step_index}: {step_name} -> {step_status}"
            )
